...
Cohort age is not tracked in default FATES. In order to have age-dependent mortality on, the flag hlm_use_cohort_age_tracking must be turned to .true. in the FATES namelist options. To set either size- or age-dependent mortality the :math:`p` and :math:`r` parameters must be set to sensible values in the FATES parameter file. 
