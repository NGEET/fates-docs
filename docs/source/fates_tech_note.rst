...
:math:`M_{hf,coh} = 0`
when the temperature of any soil layer (:math:`t_soisno_sl`) falls below -2 degrees C. 
...
.. math::
   ...
   S_{m,ft}& \textrm{for } \beta_{ft} < 10^{-6}  \textrm{and } min(t_soisno_sl) >= -2.0\\
   &\\
   0.0& \textrm{for } \beta_{ft}>= 10^{-6} \textrm{and } min(t_soisno_sl) <  -2.0.\\
   \end{array} \right.
